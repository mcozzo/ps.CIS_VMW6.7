...
Write-host ("expected Value = " + $VAR ) -ForegroundColor Yellow 
...
Get-VMHost | Foreach { 
    Get-AdvancedSetting -Entity $_ -name UserVars.ESXiShellInteractiveTimeOut | select Entity, Value #Format-List -property * # 
}
...
If($Remediate) {
    ...
    Get-VMHost | Get-AdvancedSetting -Name 'UserVars.ESXiShellInteractiveTimeOut' | Set-AdvancedSetting -Value:$VAR -Confirm:$false
    #
}
