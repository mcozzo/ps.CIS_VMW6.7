$Value = $false
$Setting = 'isolation.tools.setGUIOptions.enable'

## List the var Settings for all hosts
Write-host ("expected missing or, " + $Setting + " = " + $Value ) -ForegroundColor Yellow 
...
If($Remediate) {
    ...
    Get-VM | Get-AdvancedSetting -Name:$Setting | Set-AdvancedSetting -Value:$Value -Confirm:$false
    #
}
